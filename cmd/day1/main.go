...
import (
	...
	"github.com/dzsak/advent-of-code-2023/pkg/utils"
)
...
func main() {
	...
	res := calibrationValues(lines)
	fmt.Printf("Day 1 Part One result: %d", res)
	fmt.Println("")
	res = calibrationLetterValues(lines)
	fmt.Printf("Day 1 Part Two result: %d", res)
}
...
func calibrationValues(input []string) int {
	...
	for _, line := range input {
		calibration := calculateCalibration(line)
		sum += calibration
	}
...
func calculateCalibration(line string) int {
	...
	if len(digits) == 1 {
		return digits[0]*10 + digits[0] // For a single digit, repeat to create a two-digit number
	} else if len(digits) >= 2 {
		return digits[0]*10 + digits[len(digits)-1] // Combine first and last digits to form a two-digit number
	}
...
func calibrationLetterValues(input []string) int {
	result := 0
